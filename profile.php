...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($firstName) || empty($lastName)) {
        ...
        if (!empty($password)) {
            ...
            if ($stmt->execute([$firstName, $lastName, $hashedPassword, $userId])) {
            ...
            if ($stmt->execute([$firstName, $lastName, $userId])) {
    ...
    <main class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 w-full relative z-10">
        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <aside class="w-full md:w-64 flex-shrink-0">
                <div class="glass-card rounded-2xl p-4 sticky top-28 border border-slate-700/50">
                    <div class="text-center mb-6 pt-4">
                        <div class="w-20 h-20 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mx-auto mb-3 shadow-lg shadow-indigo-500/20 text-3xl font-bold text-white">
                            <?= strtoupper(substr(htmlspecialchars($user['first_name'] ?? 'U'), 0, 1)) ?>
                        </div>
                        <h3 class="text-lg font-bold text-white"><?= htmlspecialchars($user['first_name'] . ' ' . $user['last_name']) ?></h3>
                        <p class="text-xs text-slate-400 mt-1"><?= htmlspecialchars($user['email']) ?></p>
                    </div>
                    
                    <nav class="space-y-2">
                        <button onclick="switchTab('overview')" id="tab-btn-overview" class="w-full flex items-center space-x-3 px-4 py-3 rounded-xl bg-indigo-500/10 text-indigo-400 transition-colors text-sm font-medium border border-indigo-500/20">
                            <i class="fa-solid fa-list-ul w-5"></i>
                            <span>Order History</span>
                        </button>
                        <button onclick="switchTab('settings')" id="tab-btn-settings" class="w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-slate-400 hover:bg-slate-800/50 hover:text-white transition-colors text-sm font-medium border border-transparent">
                            <i class="fa-solid fa-gear w-5"></i>
                            <span>Settings</span>
                        </button>
                        <div class="pt-4 mt-4 border-t border-slate-800/50">
                            <a href="logout.php" class="w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 transition-colors text-sm font-medium border border-transparent">
            ...
            <div class="flex-grow space-y-6">
                ...
                <div id="tab-overview" class="block">
                    ...
                    <?php if (empty($orders)): ?>
                        <div class="glass-card rounded-2xl p-12 text-center border border-slate-700/50">
                            <div class="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-700/50">
                                <i class="fa-solid fa-box-open text-2xl text-slate-500"></i>
                            </div>
                            <h3 class="text-lg font-medium text-white">No orders yet</h3>
                            <p class="text-slate-400 text-sm mt-2 mb-6">When you place orders, they will appear here.</p>
                            <a href="products.php" class="btn-gradient text-white text-sm font-medium px-6 py-2.5 rounded-full inline-block shadow-lg shadow-indigo-500/25">Start Shopping</a>
                        </div>
                    <?php else: ?>
                        <div class="glass-card rounded-2xl overflow-hidden border border-slate-700/50 shadow-xl">
                                    ...
                                    <tbody class="text-sm">
                                        <?php foreach ($orders as $order): ?>
                                            <tr class="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                                                <td class="py-4 px-6 text-white font-medium">#<?= str_pad($order['id'], 5, '0', STR_PAD_LEFT) ?></td>
                                                <td class="py-4 px-6 text-slate-300"><?= date('M j, Y', strtotime($order['order_date'])) ?></td>
                                                <td class="py-4 px-6">
                                                    <?php 
                                                ...
                                                <td class="py-4 px-6 text-right font-bold text-white flex items-baseline justify-end gap-1">
                                                    <span class="text-indigo-400 text-xs">$</span><?= number_format($order['total_amount'], 2) ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                ...
                <div id="tab-settings" class="hidden">
                    ...
                    <div class="glass-card rounded-2xl p-6 md:p-8 border border-slate-700/50 shadow-xl">
                        <?php if ($successMsg): ?>
                        ...
                        <form method="POST" class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="first_name" value="<?= htmlspecialchars($user['first_name'] ?? '') ?>" required
                                        class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner">
                                </div>
                                <div class="space-y-2">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="email" value="<?= htmlspecialchars($user['email'] ?? '') ?>" disabled
                                    class="w-full bg-slate-800/50 border border-slate-700/50 rounded-xl px-4 py-3 text-slate-400 cursor-not-allowed shadow-inner">
                                <p class="text-xs text-slate-500 mt-1"><i class="fa-solid fa-lock text-[10px] mr-1"></i>Email address cannot be changed.</p>
                            </div>
                            ...
                            <div class="pt-4 flex justify-end">
                                <button type="submit" name="update_profile" class="btn-gradient text-white font-medium px-8 py-3 rounded-xl shadow-lg shadow-indigo-500/25 flex items-center space-x-2 transition-transform hover:-translate-y-0.5">
